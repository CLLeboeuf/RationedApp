...
public abstract class PantryItemDatabase extends RoomDatabase {
    public abstract PantryItemDao pantryItemDao();
    private static PantryItemDatabase INSTANCE;
    ...
    public static synchronized PantryItemDatabase getInstance(final Context context) {
        if (INSTANCE == null) {
            INSTANCE = Room.databaseBuilder(context.getApplicationContext(), PantryItemDatabase.class, "PantryItemDatabase").fallbackToDestructiveMigration().build();
        }
        return INSTANCE;
    }
}
